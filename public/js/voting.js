                ...
                <div class="proposal-header" onclick="toggleProposal('${proposal.id}')">
                    <div class="proposal-info">
                        ...
                        <div class="proposal-meta">
                            <span><i class="ri-user-line"></i> ${escapeHtml(proposal.profiles?.username || 'Unknown')}</span>
                            <span><i class="ri-calendar-line"></i> ${new Date(proposal.created_at).toLocaleDateString()}</span>
                            ${proposal.category ? `<span><i class="ri-folder-line"></i> ${escapeHtml(proposal.category)}</span>` : ''}
                        </div>
                    </div>
                ...
                <div class="proposal-content">
                    ${proposal.image_url ? `
                        <div style="margin-bottom: 2rem; border-radius: 12px; overflow: hidden; border: 2px solid rgba(0, 203, 151, 0.2);">
                            <img src="${escapeHtml(proposal.image_url)}" alt="${escapeHtml(proposal.title)}" style="width: 100%; max-height: 400px; object-fit: cover; display: block;">
                        </div>
                    ` : ''}

                    <div class="proposal-description">${sanitizeUserContent(proposal.description)}</div>
        ...
        modal.innerHTML = `
            <div class="bg-gray-800 rounded-2xl p-8 max-w-3xl w-full mx-4 shadow-2xl border-2 max-h-[90vh] overflow-y-auto"
                ...
                <div class="mb-6 p-4 rounded-xl" style="background: rgba(242, 195, 0, 0.1);">
                    <h3 class="font-bold text-lg mb-2">${escapeHtml(proposal.title)}</h3>
                    <p class="text-sm text-gray-400">by ${escapeHtml(proposal.profiles?.username || 'Unknown')}</p>
                </div>
                        ...
                        <div class="p-4 rounded-xl" style="background: rgba(255, 255, 255, 0.05);">
                            <div class="flex items-center gap-2 mb-2">
                                <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold"
                                    ...
                                    ${escapeHtml(comment.profiles?.username?.charAt(0).toUpperCase() || 'U')}
                                </div>
                                <div>
                                    <p class="font-semibold text-sm">${escapeHtml(comment.profiles?.username || 'Unknown')}</p>
