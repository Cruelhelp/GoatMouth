...
class MarketPage {
    ...
    renderActivityItem(bet) {
        ...
        const sharesValue = bet.shares ?? bet.potential_return ?? 0;
        const shares = parseFloat(sharesValue).toFixed(2);
    ...
    updateBetSummary() {
        ...
        if (summaryShares) summaryShares.textContent = '...';
        if (summaryPrice) summaryPrice.textContent = '...';
        if (summaryProfit) summaryProfit.textContent = '...';
    ...
    applyQuoteToSummary(quote, amount) {
        const summaryShares = document.getElementById('summary-shares');
        const summaryPrice = document.getElementById('summary-price');
        const summaryProfit = document.getElementById('summary-profit');
        const placeBtn = document.getElementById('place-bet-btn');
        ...
        if (summaryShares) summaryShares.textContent = `J$${payout.toFixed(2)}`;
        if (summaryPrice) summaryPrice.textContent = oddsFormatted;
        if (summaryProfit) summaryProfit.textContent = `+J$${profit.toFixed(2)}`;
    ...
    applyFallbackSummary(amount) {
        ...
        if (summaryShares) summaryShares.textContent = `J$${payout.toFixed(2)}`;
        if (summaryPrice) summaryPrice.textContent = this.formatDecimalOdds(odds);
        if (summaryProfit) summaryProfit.textContent = `+J$${profit.toFixed(2)}`;
    ...
    async handlePlaceBet() {
        ...
        try {
            ...
            await this.api.placeBetWithOdds(this.marketId, this.selectedOutcome, amount);
