...
class GoatMouth {
    ...
    async init() {
        ...
        // Auto-open auth modal if redirected with ?auth=login|signup
        ...
        if (authMode) {
            ...
            const cleanedQuery = authParams.toString();
    ...
    navigate(view) {
        if (this.mobileSearch && this.mobileSearch.currentOverlay) {
            this.mobileSearch.close();
        }

        this.currentView = view;
        this.currentCategory = 'all';
        if (['portfolio', 'markets', 'voting', 'leaderboard', 'activity', 'earn'].includes(view)) {
    ...
    async render() {
        ...
        if (['portfolio', 'markets', 'voting', 'leaderboard', 'activity', 'earn'].includes(hashView)) {
            ...
            categoryNav.innerHTML = `
                <div class="category-nav-mobile border-t border-gray-800 bg-gray-900" style="position: fixed; top: 124px; left: 0; right: 0; z-index: 40; width: 100%;">
                    <div class="flex gap-3 overflow-x-auto px-4 py-3" style="-webkit-overflow-scrolling: touch; scrollbar-width: none; -ms-overflow-style: none;">
                        ...
                        ` : ''}
                        <a href="#" class="mobile-card whitespace-nowrap px-4 py-2 rounded-full text-xs font-semibold transition active:scale-95 ${this.currentView === 'earn' ? 'category-active' : ''}"
                           style="background: ${this.currentView === 'earn' ? '#F2C300' : 'rgba(242, 195, 0, 0.1)'};
                                  color: ${this.currentView === 'earn' ? 'black' : '#F2C300'};
                                  border: 1px solid ${this.currentView === 'earn' ? '#F2C300' : 'transparent'};"
                           data-nav="earn">
                            Earn
                        </a>
    ...
    renderMarketCard(market) {
        ...
        const yesOddsDisplay = market.yes_price ? `${(1 / market.yes_price).toFixed(2)}x` : '--';
        const noOddsDisplay = market.no_price ? `${(1 / market.no_price).toFixed(2)}x` : '--';
            ...
            return `
                <div class="mobile-card bg-gray-800 rounded-xl overflow-hidden border border-gray-700"
                    ...
                    <div onclick="window.location.href='market.html?id=${market.id}'" style="cursor: pointer; position: relative;">
                        ...
                        <button onclick="event.stopPropagation(); app.shareMarket('${market.id}', '${escapeHtml(market.title)}');"
                                class="share-btn"
                        ...
                        <div class="p-4">
                            ...
                            <div class="flex items-center justify-between mb-2">
                                ${market.category ? `
                                    <span class="text-xs font-semibold" style="color: #059669;">${escapeHtml(market.category)}</span>
                                ` : '<span></span>'}
                            ...
                            <div class="quick-bet-actions grid grid-cols-2 gap-2 mb-3" data-quick-bet-actions>
                                <button onclick="event.stopPropagation(); app.quickBet('${market.id}', 'yes', ${market.yes_price});"
                                    ...
                                    <span class="text-lg font-bold" style="color: #059669;">${yesOddsDisplay}</span>
                                </button>
                                ...
                                <button onclick="event.stopPropagation(); app.quickBet('${market.id}', 'no', ${market.no_price});"
                                    ...
                                    <span class="text-lg font-bold" style="color: #ef4444;">${noOddsDisplay}</span>
                                </button>
                            </div>
                        ...
                        <div class="p-3">
                            ...
                            <div class="quick-bet-actions grid grid-cols-2 gap-2 mb-3" data-quick-bet-actions>
                                <button onclick="event.stopPropagation(); app.quickBet('${market.id}', 'yes', ${market.yes_price});"
                                    ...
                                    <span class="text-base font-bold" style="color: #059669;">${yesOddsDisplay}</span>
                                </button>
                                ...
                                <button onclick="event.stopPropagation(); app.quickBet('${market.id}', 'no', ${market.no_price});"
                                    ...
                                    <span class="text-base font-bold" style="color: #ef4444;">${noOddsDisplay}</span>
                                </button>
                            </div>
                ...
                <div style="padding: 12px; display: flex; align-items: center; gap: 8px; overflow: hidden;">
                    ...
                    ${market.image_url ? `
                        <div class="flex-shrink-0">
                            <img src="${escapeHtml(market.image_url)}"
                                 alt="${escapeHtml(market.title)}"
    ...
    renderEarn(container) {
        container.innerHTML = `
            <p class="text-center py-8">Please sign in to view activity</p>
        `;
    }
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <div class="flex items-center gap-2">
                                                    <div class="flex items-center gap-1 px-2 py-1 rounded" style="background: rgba(5, 150, 105, 0.12);">
                                                        <span class="text-xs font-semibold" style="color: #059669;">YES</span>
                                                        <span class="text-sm font-bold" style="color: #059669;">${yesOddsDisplay}</span>
                                                    </div>
                                                    <div class="flex items-center gap-1 px-2 py-1 rounded" style="background: rgba(239, 68, 68, 0.12);">
                                                        <span class="text-xs font-semibold" style="color: #ef4444;">NO</span>
                                                        <span class="text-sm font-bold" style="color: #ef4444;">${noOddsDisplay}</span>
                                                    </div>
                                                </div>
                                            </div>
