...
class AdminPanel {
    ...
    showCreateMarketModal() {
        ...
        modal.innerHTML = `
            <div class="admin-modal-content">
                ...
                <form id="create-market-form" class="p-6 space-y-4">
                    ...
                    <div class="form-group">
                        <label class="form-label">Odds Guidance</label>
                        <div class="bg-gray-800 border border-gray-700 rounded-lg p-4 text-sm">
                            <div id="odds-guidance-status" class="text-gray-300 mb-2">Loading guidance...</div>
                            <div class="grid grid-cols-2 gap-2 text-xs text-gray-400">
                                <div>Suggested YES: <span id="odds-guidance-yes" class="text-white font-semibold">--</span></div>
                                <div>Suggested NO: <span id="odds-guidance-no" class="text-white font-semibold">--</span></div>
                                <div>Confidence: <span id="odds-guidance-samples" class="text-white font-semibold">--</span></div>
                                <div id="odds-guidance-source">Source: --</div>
                            </div>
                            <div class="text-xs text-gray-500 mt-2">Use as a baseline reference before market opens.</div>
                        </div>
                    </div>
                    <div class="form-group">
        ...
        if (categorySelect) {
            this.updateGuidancePreview(categorySelect.value, modal);
    ...
    async renderApiInsights(container) {
        const [configRes, logsRes, healthRes, statsRes, overridesRes, marketsRes] = await Promise.all([
            ...
            window.supabaseClient
                ...
                .limit(20),
            window.supabaseClient
                .from('odds_guidance_category_stats')
                .select('*')
        ...
        this.oddsGuidanceConfig = config;
        ...
        container.innerHTML = `
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-icon"><i class="fa-solid fa-wave-square"></i></div>
                    <h3>Recent Requests</h3>
                    <div class="value">${totalLogs}</div>
                    <div class="label">Last 50 odds API calls</div>
                </div>
                <div class="stat-card">
                    <div class="stat-icon"><i class="fa-solid fa-triangle-exclamation"></i></div>
                    <h3>Error Rate</h3>
                    <div class="value">${totalLogs ? Math.round((errorLogs / totalLogs) * 100) : 0}%</div>
                    <div class="label">${errorLogs} errors</div>
                </div>
                <div class="stat-card">
                    <div class="stat-icon"><i class="fa-solid fa-stopwatch"></i></div>
                    <h3>Avg Latency</h3>
                    <div class="value">${avgLatency}ms</div>
                    <div class="label">Last 50 requests</div>
                </div>
                <div class="stat-card">
                    <div class="stat-icon"><i class="fa-solid fa-heart-pulse"></i></div>
                    <h3>Last Health</h3>
                    <div class="value" style="font-size: 20px;">${lastHealthLabel}</div>
                    <div class="label">Odds API status</div>
                </div>
            </div>
            ...
            <div class="card">
                <h3>Manual Guidance Overrides</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
                    <input id="override-category" class="form-input" placeholder="Category name">
                    <input id="override-yes-prob" class="form-input" type="number" min="0" max="1" step="0.01" placeholder="YES probability (0-1)">
                    <button class="btn secondary" id="save-guidance-override"><i class="fa-solid fa-wand-magic-sparkles"></i> Save Override</button>
                </div>
                <div class="table-container">
                    <table class="data-table">
                        <thead>
                        ...
                        <tbody>
                            ${overrides.length ? overrides.map(row => `
                                <tr>
                                    <td class="font-semibold">${row.scope_value}</td>
                                    <td>${this.formatPercent(row.yes_probability)}</td>
                                    <td>${row.enabled ? 'Enabled' : 'Disabled'}</td>
                                    <td>
                                        <button class="btn secondary" data-override-toggle="${row.id}" data-override-enabled="${row.enabled}">
                                            ${row.enabled ? 'Disable' : 'Enable'}
                                        </button>
                                        <button class="btn danger" data-override-delete="${row.id}">Delete</button>
                                    </td>
                                </tr>
                            `).join('') : `
                                <tr><td colspan="4" class="text-center text-gray-400 py-6">No overrides yet</td></tr>
                            `}
                        </tbody>
                    </table>
                </div>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            ${healthChecks.length ? healthChecks.map(check => `
                                <tr>
                                    <td class="font-semibold">${check.status}</td>
                                    <td>${check.latency_ms || 0}ms</td>
                                    <td>${new Date(check.created_at).toLocaleString()}</td>
                                    <td class="text-sm text-gray-400">${check.error_message || '-'}</td>
                                </tr>
                            `).join('') : `
                                <tr><td colspan="4" class="text-center text-gray-400 py-6">No health checks yet</td></tr>
                            `}
                        </tbody>
                    </table>
                </div>
                        ...
                        <tbody>
                            ${stats.length ? stats.map(row => `
                                <tr>
                                    ...
                                    <td>${this.formatPercent(row.yes_rate)}</td>
                                    <td>${this.formatOdds(row.yes_rate)}</td>
                                    <td>${new Date(row.last_updated).toLocaleString()}</td>
                                </tr>
                            `).join('') : `
                                <tr><td colspan="5" class="text-center text-gray-400 py-6">No guidance stats yet</td></tr>
                            `}
                        </tbody>
                    </table>
                </div>
        ...
        const recomputeBtn = container.querySelector('#recompute-guidance');
        const saveOverrideBtn = container.querySelector('#save-guidance-override');

        if (saveBtn) {
            saveBtn.addEventListener('click', () => this.saveApiConfig());
        }
        if (healthBtn) {
            healthBtn.addEventListener('click', () => this.runOddsApiHealthCheck());
        }
        if (recomputeBtn) {
            recomputeBtn.addEventListener('click', () => this.recomputeGuidanceStats());
        }
        if (saveOverrideBtn) {
            saveOverrideBtn.addEventListener('click', () => this.saveGuidanceOverride());
        }

        container.querySelectorAll('[data-override-toggle]').forEach(btn => {
            btn.addEventListener('click', () => {
                const overrideId = btn.getAttribute('data-override-toggle');
                const enabled = btn.getAttribute('data-override-enabled') === 'true';
                this.toggleGuidanceOverride(overrideId, !enabled);
            });
        });

        container.querySelectorAll('[data-override-delete]').forEach(btn => {
            btn.addEventListener('click', () => {
                const overrideId = btn.getAttribute('data-override-delete');
                this.deleteGuidanceOverride(overrideId);
            });
        });
    }

    async saveApiConfig() {
        const loggingEnabled = document.getElementById('config-logging-enabled')?.checked ?? true;
        const guidanceEnabled = document.getElementById('config-guidance-enabled')?.checked ?? true;
        const retentionDays = Number(document.getElementById('config-retention-days')?.value || 30);
        const smoothingAlpha = Number(document.getElementById('config-smoothing-alpha')?.value || 1);
        const globalPrior = Number(document.getElementById('config-global-prior')?.value || 0.5);
    ...
    async runOddsApiHealthCheck() {
        const oddsApiUrl = window.ODDS_API_URL || 'http://localhost:3001';
        const start = performance.now();

        try {
            const response = await fetch(`${oddsApiUrl}/health`, { cache: 'no-store' });
            const latency = Math.round(performance.now() - start);
            const data = await response.json().catch(() => null);

            const status = response.ok && data?.status === 'ok' ? 'ok' : 'error';
            const errorMessage = response.ok ? null : data?.error?.message || response.statusText;
            ...
            await window.supabaseClient
                ...
                .insert({
                    ...
                    response_meta: data,
                    checker: this.currentProfile?.username || 'admin'
                });

            this.showToast('Health check recorded', status === 'ok' ? 'success' : 'error');
            this.renderView('api');
        } catch (error) {
            await window.supabaseClient
                .from('odds_api_health_checks')
    ...
    async recomputeGuidanceStats() {
        const smoothingAlpha = Number(document.getElementById('config-smoothing-alpha')?.value || 1);
        const { error } = await window.supabaseClient
            .rpc('recompute_odds_guidance_stats', { p_smoothing: smoothingAlpha });

        if (error) {
            this.showToast(`Failed to recompute guidance: ${error.message}`, 'error');
            return;
        }

        this.showToast('Guidance stats refreshed', 'success');
        this.renderView('api');
    }

    async saveGuidanceOverride() {
        const category = document.getElementById('override-category')?.value?.trim();
        const yesProbability = Number(document.getElementById('override-yes-prob')?.value);

        if (!category || Number.isNaN(yesProbability) || yesProbability < 0 || yesProbability > 1) {
            this.showToast('Enter a category and YES probability between 0 and 1', 'error');
            return;
        }
        ...
        const { error } = await window.supabaseClient
            ...
            .upsert({
                ...
                enabled: true,
                created_by: this.currentProfile?.id || null,
                created_at: new Date().toISOString()
            }, { onConflict: 'scope_type,scope_value' });

        if (error) {
            this.showToast(`Failed to save override: ${error.message}`, 'error');
            return;
        }
    ...
    async toggleGuidanceOverride(overrideId, enabled) {
        const { error } = await window.supabaseClient
            .from('odds_guidance_overrides')
            .update({ enabled })
            .eq('id', overrideId);
        ...
        if (error) {
            this.showToast(`Failed to update override: ${error.message}`, 'error');
            return;
        }
    ...
    async deleteGuidanceOverride(overrideId) {
        ...
        if (error) {
            this.showToast(`Failed to delete override: ${error.message}`, 'error');
            return;
        }
    ...
    async getGuidanceForCategory(category) {
        let config = this.oddsGuidanceConfig;
        if (!config) {
            const { data } = await window.supabaseClient
                ...
                .eq('id', 1)
                .single();
            config = data || { guidance_enabled: true, global_prior: 0.5 };
            this.oddsGuidanceConfig = config;
        }
        ...
        const { data: override } = await window.supabaseClient
            .from('odds_guidance_overrides')
            .select('*')
            .eq('enabled', true)
            .eq('scope_type', 'category')
            .eq('scope_value', category)
            .maybeSingle();

        if (override?.yes_probability) {
            return {
                enabled: true,
                yesRate: override.yes_probability,
                sampleSize: null,
                source: 'Manual override'
            };
        }

        const { data: stat } = await window.supabaseClient
            .from('odds_guidance_category_stats')
            .select('*')
        ...
        return {
            enabled: true,
            yesRate: config.global_prior || 0.5,
            sampleSize: 0,
            source: 'Global prior'
        };
    }

    async updateGuidancePreview(category, modal) {
        const statusEl = modal.querySelector('#odds-guidance-status');
        const yesEl = modal.querySelector('#odds-guidance-yes');
        const noEl = modal.querySelector('#odds-guidance-no');
        const samplesEl = modal.querySelector('#odds-guidance-samples');
        const sourceEl = modal.querySelector('#odds-guidance-source');

        if (!statusEl || !yesEl || !noEl || !samplesEl || !sourceEl) return;

        statusEl.textContent = 'Loading guidance...';

        try {
            const guidance = await this.getGuidanceForCategory(category);
            if (!guidance.enabled) {
                statusEl.textContent = 'Guidance disabled in settings';
                yesEl.textContent = '--';
                noEl.textContent = '--';
                samplesEl.textContent = '--';
                sourceEl.textContent = 'Source: disabled';
                return;
            }

            const yesRate = guidance.yesRate || 0.5;
            const noRate = 1 - yesRate;

            statusEl.textContent = 'Suggested baseline odds';
            yesEl.textContent = `${this.formatPercent(yesRate)} (${this.formatOdds(yesRate)})`;
            noEl.textContent = `${this.formatPercent(noRate)} (${this.formatOdds(noRate)})`;
            samplesEl.textContent = guidance.sampleSize !== null ? `${guidance.sampleSize} samples` : 'Manual';
            sourceEl.textContent = `Source: ${guidance.source}`;
        } catch (error) {
            statusEl.textContent = 'Unable to load guidance';
        }
    }
